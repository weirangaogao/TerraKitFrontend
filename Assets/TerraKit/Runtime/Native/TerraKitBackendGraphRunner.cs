...
namespace TerraKit
{
    ...
    public sealed partial class TerraKitBackendGraphRunner
    {
        ...
        {
            var regions = GenerateResources(graph, generationRequests);
            var meshes = new List<TerraKitGeneratedMeshData>();
            foreach (var region in regions)
            {
                var outputs = region.Resources.Where(r => r.Kind == TerraKitBackendResourceKind.Mesh).ToList();
                if (outputs.Count != 1)
                    throw new InvalidOperationException("This shortcut needs one Mesh output. Open Backend Generator to select any result.");
                meshes.Add(outputs[0].Mesh);
            }
            ...
            foreach (TerraKitNodeData node in compileResult.ExecutionOrder)
            {
                if (!TerraKitNodeRegistry.IsBackendExecutable(node.typeId))
                {
                    throw new NotSupportedException(
                        ...
                        "Unsupported node: " + node.displayName +
                        " (" + node.typeId + ").");
                }
            }
